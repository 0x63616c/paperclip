//! `bin/sudoku`, the compiled entrypoint, not just the linked `App` (§8's
//! launch-time layer, exercised across a real process boundary).
//!
//! Mirrors `apps/chess/tests/entrypoint.rs`: spawn the built binary, send it
//! a real `Hello` on its stdin, and read a real `Ready` off its stdout — the
//! artefact `apps/sudoku/paper.toml` names as its entrypoint, not just the
//! `SudokuApp` type behind it.
...
fn hello(storage: &Path) -> Hello {
    ...
    Hello {
        ...
        app: "dev.calum.sudoku".parse::<AppId>().expect("a valid app id"),
...
fn the_built_entrypoint_answers_hello_with_ready_saves_and_exits_cleanly() {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_sudoku"))
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
    ...
    match codec::read_message::<_, AppMessage>(&mut stdout).expect("reads a reply") {
        AppMessage::Saved(Saved { ok, .. }) => {
            assert!(ok, "a fresh puzzle with a writable private dir should save");
        }
