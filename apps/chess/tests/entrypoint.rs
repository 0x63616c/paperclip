//! `bin/chess`, the compiled entrypoint, not just the linked `App` (§8's
//! launch-time layer, exercised across a real process boundary).
//!
//! `session::open_session_with` (`tools/paperctl/src/session.rs`) already
//! proves `ChessApp` behind `paper_sdk::run` speaks the protocol over a
//! `UnixStream`. This proves the same thing over the actual artefact
//! `apps/chess/paper.toml` names as its entrypoint: spawn the built binary,
//! send it a real `Hello` on its stdin, and read a real `Ready` off its
//! stdout.
...
fn hello(storage: &Path) -> Hello {
    ...
    Hello {
        ...
        app: "dev.calum.chess".parse::<AppId>().expect("a valid app id"),
...
fn the_built_entrypoint_answers_hello_with_ready_saves_and_exits_cleanly() {
    ...
    match codec::read_message::<_, AppMessage>(&mut stdout).expect("reads a reply") {
        AppMessage::Saved(Saved { ok, .. }) => {
            assert!(ok, "a fresh game with a writable private dir should save");
        }
