...
| A4 | A greyscale UI is the safe default; colour is a bonus, not a dependency. | ADR-0005; `paper_sdk::palette` | **confirmed, for a different reason than the one given** | The panel is E Ink Gallery 3 colour (`EPFramebufferAcep2`). **The 16-level reasoning is withdrawn** along with the 4bpp premise (ADR-0007): grey levels are not scarce. What makes greyscale-first right is that the *fast* waveforms are monochrome — `Waveform::INK` is mono mode 0 — so anything that must respond quickly is grey whatever the panel can display. Colour is a settled-UI choice, encoded as `Waveform::CONTENT` in `paper_device::waveform`. |
| A5 | Pointer input arrives as press / move / release with a position. | `paper_protocol::PointerEvent`, re-exported as `paper_sdk::PointerEvent` | **closed — refuted, then redesigned (WWW-5)** | Refuted by WWW-1 §3: the device reports pen `ABS_PRESSURE` 0–4096, `ABS_DISTANCE` (hover), `ABS_TILT_X/Y` ±9000, `BTN_TOOL_RUBBER` for the eraser, and touch as multitouch protocol B with **10 simultaneous slots**. WWW-5 redesigned the type against those facts: normalized `Pressure` and `Tilt` as `Option`s, a platform-allocated `ContactId` per contact, `Pointer::Eraser`, and `PointerPhase::Hover`. **Hover *distance* is deliberately still absent** — WWW-1 recorded the axis without its range, so there is no honest way to normalize it (ADR-0016). |
...
Facts established after Stage 1 was written that Stage 1's code did not yet
account for. None of them is a Stage 1 defect; they are the inputs WWW-3 and
WWW-5 designed against.
...
- **Grey levels are not scarce, and the palette question changed.** This is a
  ...
  candidate modes have names — `Waveform::INK`, `Waveform::ANIMATION`,
  `Waveform::UI`, `Waveform::CONTENT` in `paper_device::waveform` — and, as of
  WWW-35, **measured** `sync()` completion times against the vendor's own
  enum (`EngineMode`): Pen 372ms, Mono 634ms, Animation 304ms (fastest, not
  yet used by any `Waveform` constant), Ui 630ms, Content 1086ms (slowest).
  `MONO_QUALITY` and `COLOR` were this file's own names for modes 3 and 4;
  WWW-35 corrected them to the vendor's `Ui` and `Content`. The light end of
  the palette still needs a legibility check on hardware.
