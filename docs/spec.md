...
| §4 (device transport) | [ADR-0020](adr/0020-mac-side-device-transport.md) | `open`, `stock`, `setup`, `install`, `upgrade` and `remove` typed on a Mac resolve a tablet (`--device`, `PAPERCTL_DEVICE`, a pin, USB, mDNS, then a cached last-good host) and forward to the tablet's own `paperctl` over SSH; `paperctl devices` lists and pins. mDNS is unvalidated on hardware. WWW-33, 2026-09-17. |
| §4 (`paperctl logs`, `doctor`, `deploy`) | [ADR-0023](adr/0023-paperctl-logs-doctor-deploy.md) | Three commands replace a set of personal shell aliases: `logs` reads a Mac-side run history `open`/`deploy` write; `doctor` reports device reachability, `xochitl.service` health, the vendor lock and the on-device `paperctl` version, exiting 0/1/2 for healthy/degraded/unreachable; `deploy` cross-compiles and installs `paperctl` itself over the WWW-33 transport. `doctor` and `deploy` resolve through a 4s budget, not `open`'s 30s one. WWW-34, 2026-09-17. |
